...
class Step extends EventTarget  {
    ...
    getStepMarkup():string {
        return `<div class="step-header">${(this.id ?? 0) + 1 } sur ${this.amount}</div><div class="step-label">${this.label}</div>
            <div class="step-footer">
                <svg class="bt-previous" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    set(newId?:number, totalStepsAmount?:number):void{
        ...
        if( this.isSet !== true ){
            ...
            this.markup.querySelector('.bt-previous')?.addEventListener("pointerup", (click:Event):void => {
